...
function duniang_side_comm() {
	...
	foreach ($comments as $comment) {
		$s .= '<li><h4 class="pl-list"><a href="'.$comment->Post->Url.'">'.$comment->Post->Title.'</a><small> 的评论：</small></h4>';
		$s .= TransferHTML($comment->Content,'[noenter]').'<br>';
		$s .= '<a href="'. $comment->Post->Url.'" class="text-success">'.$comment->Author->StaticName.'</a>';
		$s .= '<time class="text-muted"> - '.$comment->Time('Y-m-d').'</time></li>';
	}
	return $s;
}
